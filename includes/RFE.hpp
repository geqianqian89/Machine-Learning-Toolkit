...
private:
    ...
    RFE(std::shared_ptr<Data< T > > samples = nullptr, Classifier< T >* classifier = nullptr, Validation< T >::CrossValidation *cv = nullptr, int depth = 0, int skip = 0, int jump = 0, bool leave_one_out = false);
